...
def fetch_all(groups_csv: str, years: int = 3) -> dict:
    ...
    for sector in sectors:
        frames = []
        for t in df.query("sector == @sector")["ticker"].unique():
            sub = dl.taiwan_stock_month_revenue(stock_id=str(t), start_date=(datetime.today().date().replace(day=1).replace(year=datetime.today().year-years)).isoformat())
            if sub.empty:
                continue
            frames.append(
                sub.rename(columns={
